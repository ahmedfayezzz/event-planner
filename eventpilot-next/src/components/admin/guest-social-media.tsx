...
interface GuestSocialMediaProps {
  guestId: string;
  socialMediaLinks?: Record<string, string> | null;
  onUpdate?: () => void;
}

export function GuestSocialMedia({
  guestId,
  socialMediaLinks,
  onUpdate,
}: GuestSocialMediaProps) {
  ...
  const updateMutation = api.guest.updateSocialMedia.useMutation({
    onSuccess: () => {
      onUpdate?.();
      utils.guest.getById.invalidate({ id: guestId });
  ...
  const handleSave = () => {
    // Check if there's a pending link that hasn't been added
    if (newPlatform && newUrl.trim()) {
      toast.error("لديك رابط لم تتم إضافته. اضغط على زر (+) لإضافته أولاً", {
        duration: 4000,
      });
      return;
    }
    ...
    updateMutation.mutate({
      id: guestId,
      socialMediaLinks: Object.keys(links).length > 0 ? links : null,
    });
  };
  ...
  const getPlatformLabel = (platform: string) => {
    return (
      SOCIAL_MEDIA_PLATFORMS.find((p) => p.key === platform)?.label ?? platform
    );
  };
    ...
    <div className="space-y-4">
      ...
      {linkCount === 0 ? (
        <div className="text-center py-6 text-muted-foreground">
          ...
          <p className="text-sm">لا توجد روابط تواصل</p>
        </div>
      ) : (
              ...
              Object.entries(links).map(([platform, url]) => (
                ...
                >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 w-0 text-xs text-muted-foreground hover:text-primary flex items-center gap-1"
                    title={url}
                  >
                  ...
                  <button
                    ...
                    className="shrink-0 text-destructive hover:text-destructive/80 p-1 rounded hover:bg-destructive/10"
                  >
...
// Display-only component for showing social media icons
export function GuestSocialMediaIcons({
  socialMediaLinks,
}: {
  socialMediaLinks?: Record<string, string> | null;
}) {
