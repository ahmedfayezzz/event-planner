...
interface SponsorLabelManagerProps {
  sponsorId: string;
  sponsorLabels: Array<{ id: string; name: string; color: string }>;
...
export function SponsorLabelManager({
  sponsorId,
  sponsorLabels,
  trigger,
  onUpdate,
}: SponsorLabelManagerProps) {
  const [open, setOpen] = useState(false);
  const [searchValue, setSearchValue] = useState("");
  // Use sponsorLabels directly as source of truth, local state only for optimistic updates
  const [localLabels, setLocalLabels] = useState<string[] | null>(null);

  // Use local state if set, otherwise use prop
  const selectedLabels = localLabels ?? sponsorLabels.map((l) => l.id);

  const { data: allLabels, refetch: refetchLabels } = api.sponsor.getLabels.useQuery();

  const createLabelMutation = api.sponsor.createLabel.useMutation({
  ...
  const assignLabelsMutation = api.sponsor.updateSponsorLabels.useMutation({
    onSuccess: () => {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>تصنيفات الراعي</DialogTitle>
        </DialogHeader>
